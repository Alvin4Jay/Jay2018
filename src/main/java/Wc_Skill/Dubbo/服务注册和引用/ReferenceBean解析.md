## ReferenceBean解析——xml配置的情况，以ReferenceConfig为基础

<<dubbo:reference/>> xml自定义标签被spring加载时生成ReferenceBean。
...
public class ReferenceBean<T> extends ReferenceConfig<T>
        implements ApplicationContextAware, FactoryBean, InitializingBean, DisposableBean {
    ...
    public void setApplicationContext(ApplicationContext applicationContext) {
        this.applicationContext = applicationContext;  //设置应用上下文
        SpringExtensionFactory.addApplicationContext(applicationContext); //SpringExtensionFactory添加context,用于后续从SpringExtensionFactory中获取Bean.
    }
    ...
    public Class<?> getObjectType() {
        return getInterfaceClass(); //获取服务接口类
    }
    ...
    /// InitializingBean  设置各种config，并进行服务代理对象的初始化
    @Override
